...
const ConversationEvent = {
    ...
    TTR_CHANGE: 'ttr-change',
    MANUAL_ETTR_CHANGE: 'manual-ettr-change'
};
...
class Conversation extends EventEmitter {
    ...
    _initFromConversationState(conversationState) {
        ...
        this.ttrEffectiveManual = convDetails.manualETTR || null;
    ...
    processStateNotification(state) {
        ...
        if (!this._receivedInitialState) {
            ...
            this.emit(ConversationEvent.NOTIFICATION, state);
            return;
        }
        ...
        if (newStateHash === this._prevStateHash) {
            ...
            this.emit(ConversationEvent.NOTIFICATION_IGNORED, state);
            return;
        }
        ...
        if (this.updateTime > state.conversationDetails.metaDataLastUpdateTs) {
            ...
            this.emit(ConversationEvent.NOTIFICATION_OUT_OF_ORDER, state);
            return;
        }
        ...
        if (state.conversationDetails.groupId && this.groupId !== state.conversationDetails.groupId) {
            ...
            this.emit(ConversationEvent.GROUP_CHANGE, { prevGroupId: this.groupId, newGroupId: state.conversationDetails.groupId });
        ...
        if (state.conversationDetails.skillId && this.skill.skillId !== state.conversationDetails.skillId) {
            ...
            this.emit(ConversationEvent.SKILL_CHANGE, { prevSkillId: state.conversationDetails.skillId, newSkillId: this.skill.skillId });
        ...
        if (state.conversationDetails.stage && this.stage !== state.conversationDetails.stage) {
            ...
            if (this.stage === ConversationState.CLOSE) {
                ...
                this.emit(ConversationEvent.CLOSE);
            }
        }
        ...
        if (state.conversationDetails.ttr) {
            ...
            // if either of the above changed, emit
            if (ttrChanged) {
                this.emit(ConversationEvent.TTR_CHANGE, {
                    ttrValue: this.ttrValue,
                    ttrType: this.ttrType,
                    ttrEffective: this.ttrEffective
                });
            }
        }

        // -state.conversationDetails.manualETTR - number
        if (state.conversationDetails.manualETTR && this.ttrEffectiveManual !== state.conversationDetails.manualETTR) {
            this.ttrEffectiveManual = state.conversationDetails.manualETTR
            this.emit(ConversationEvent.MANUAL_ETTR_CHANGE, {
                ttrEffectiveManual: this.ttrEffectiveManual,
                ttrEffective: this.ttrEffective
            });
        }
    ...
    _processDialogStateNotification(dialogState, conversationContext) {
        ...
        if (!(dialogId in this._dialogsById)) {
            ...
            this.emit(ConversationEvent.DIALOG, dialog);
        }
        else {
     ...
     */
    unload() {
        // TODO allow this to work for any conversation regardless of source
        if (!this._manuallyLoaded) {
            throw new Error('invalid operation, conversation was not previously loaded with loadConversation');
        }
        this._decrementReferenceCounter();
    ...
    _onMessageNotification(message) {
        ...
        this.emit(ConversationEvent.MESSAGE_NOTIFICATION, message);
    }
    ...
    /**
     * Updates the conversation's note field
     * @param {string} note The note text, max length of 5,000 characters
     */
    async setNote(note) {
     ...
     * @param {TTRType} ttrType
     * @param {Number} [value] - TTR value in seconds. This value is only updatable when ttrType is CUSTOM. For all other types, the value is assigned by the system.
     * @returns {Promise<void>}
     */
    async setTTR(ttrType, value = 0) {
        await this._connection._setTTR(this.conversationId, ttrType, value);
    }

    /**
     * Sets the manual time to respond for the conversation, only supported for brand connections
     * @param {Number} time - time to respond in timestamp format
     * @returns {Promise<void>}
     */
    async setManualETTR(time) {
        await this._connection._setManualETTR(this.conversationId, time);
    }

    _isWhitelistedConversationEvent(event) {
        const whitelist = [
            ConversationEvent.GROUP_CHANGE,
            ConversationEvent.SKILL_CHANGE,
            ConversationEvent.CLOSE,
            ConversationEvent.DIALOG,
            ConversationEvent.MANUAL_ETTR_CHANGE,
            ConversationEvent.TTR_CHANGE
        ];
        return whitelist.includes(event);
    }

    /**
     * Emit whitelisted events up to the connection, if there is a connection-level listener for the event;
     * otherwise, emit it at the current level
     * @param type
     * @param args
     * @returns {*|boolean}
     * @private
     */
    _emitUp(type, ...args) {
        return this._connection.emit(`conversation:${type}`, { conversation: this, ...args }) || super.emit(type, ...args);
    }

    emit(type, ...args) {

        // bubble up to connection level
        if (this._isWhitelistedConversationEvent(type)) {
            return this._emitUp(type, ...args);
        }
     ...
     * @param {string} text - Message text, max length 8,000 characters
     * @returns {Promise<void>}
     */
     ...
     * @param {Object} message - Structured content or rich text, max length 15,000 characters
     * @returns {Promise<void>}
     */
